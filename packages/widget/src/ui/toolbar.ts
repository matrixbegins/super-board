...
export function createToolbar(shadowRoot: ShadowRoot, options: ToolbarOptions) {
  ...
  toolbar.append(toolsGroup, divider1, colorGroup, divider2, historyGroup, divider3, doneGroup);
  ...
  function setActiveColor(color: string): void {
    ...
    for (const btn of colorButtons) {
      btn.classList.toggle('kan-toolbar-color-active', btn.style.backgroundColor === color || btn.title === color);
    }
