---
title: Kan + MinIO (S3)
description: Deploy Kan with PostgreSQL and MinIO (S3-compatible storage) using Docker Compose, with clear steps and production notes.
---
...
<Cards>
  ...
  <Card title="MinIO (S3-compatible)">
...
<Callout type="warn">
  ...
  certificates, and DNS for your domains (e.g., `kan.example.com`, `s3.example.com`).
</Callout>
...
- Kan: [http://kan.localtest.me:3000](http://kan.localtest.me:3000)
- MinIO Console: [http://minio.localtest.me:9001](http://minio.localtest.me:9001)
- MinIO S3 API: [http://s3.localtest.me:9000](http://s3.localtest.me:9000)
...
<Tabs items={["Local", "Production"]}>
  <Tab value="Local">
    ...
    - `NEXT_PUBLIC_STORAGE_URL=http://s3.localtest.me:9000`
    - `NEXT_PUBLIC_STORAGE_DOMAIN=s3.localtest.me`
    - Keep `S3_FORCE_PATH_STYLE=true` for MinIO.
  </Tab>
...
<Accordions>
  ...
  <Accordion title="Make the bucket public (read-only) with mc">
